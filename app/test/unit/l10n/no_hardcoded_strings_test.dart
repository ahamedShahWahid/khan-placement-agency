...
final _patterns = [
  ...
  RegExp(r'Text\(\s*"[A-Za-z]'),
  RegExp(r"hintText:\s*'[A-Za-z]"),
  RegExp(r"labelText:\s*'[A-Za-z]"),
  RegExp(r"tooltip:\s*'[A-Za-z]"),
];
...
void main() {
  test('no hardcoded user-visible strings outside recruiter/ and allowlist',
    ...
    for (final f in dir.listSync(recursive: true).whereType<File>()) {
      ...
      for (final p in _patterns) {
        for (final m in p.allMatches(src)) {
          ...
          final snippet =
              src.substring(m.start, m.start + 30).replaceAll('\n', ' ');
    ...
    expect(
      offenders,
      isEmpty,
      reason: 'Localize via context.l10n (ARB), or allowlist with '
          'justification:\n${offenders.join('\n')}',
    );
  });
}
