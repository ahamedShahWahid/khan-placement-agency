...
Two eval lanes: CI = library at 0.85 (deterministic); acceptance 0.90 = on-demand LLM lane, record committed at `core/data/parse_eval/LLM_EVAL_REPORT.md` — refresh it in the same commit as any prompt/model/dataset change (numbers live there, not here). Two things that shape its skills number: 013/014/019 expect `[]`, so any skills the model extracts there score as FPs until a product decision on non-tech "skills" (don't "fix" the parser or the gold files for it); and the test prints a per-example skills token diff so every FP can be checked against the resume text — a token absent from the text is an invented skill, the one error class the gate exists to catch. Key failures: a 429 whose body says "prepayment credits are depleted" = top up the prepaid project (fails fast, ~1 min to propagate); a 403 = key from a project without billing, a top-up won't fix it. **There is ONE call shape — interactive.** `parse_texts_batch` (Gemini Batch API) was removed 2026-08-15: no caller once the eval lane moved to paced interactive, and never verified live (batch returned FAILED_PRECONDITION on free tier, then 403). In git history if bulk re-parse becomes real — verify it against the real API before trusting it; its only coverage was mocked.
